...
class MyHashSet {
    ...
    public int getHashKey(int key){
        return key % 1000;                  // Since the range is from 0-10^6, we use a 1000*1001 matrix
    }

    public int getHashKey2(int key){
        return key / 1001;                  // To accommodate the last value 10^6 and to avoid overflow, we use 1001, instead of 1000
    }
    ...
    public void remove(int key) {
        int hashKey = getHashKey(key);
        int hashKey2 = getHashKey2(key);
        if (matrix[hashKey] == null) return;     // In case the Nested Array is not present, we don't do anything
